# Relatório de Integração do Modelo

## Resumo Executivo

Integramos o arquivo de modelo treinado `Modelagem/Modelos/modelo_previsao_voos.bin` ao pipeline existente (Java API → `ml-wrapper` → `modelos-ml`). O arquivo .bin não era um pickle padrão; utilizamos CatBoost para carregá-lo diretamente. Ajustamos o wrapper para mapear corretamente os campos e adicionamos validações e scripts de diagnóstico para facilitar os testes.

## O que foi Alterado (Visão Geral)

### `Modelagem/Modelos/app.py`

- Carregamento robusto do modelo com fallback para detectar formatos; uso principal de `catboost` para este arquivo .bin.
...
- Removidos `lightgbm` e `xgboost` para reduzir o tamanho da imagem; mantido apenas `catboost` (necessário para este arquivo .bin).
...
- O arquivo .bin recebido não era um pickle padrão; forçar um `joblib.load` causava erros. Carregar com CatBoost preserva o modelo e evita retraining/conversão.
...
**Importante:** Ajuste a data para uma data futura válida. Caso contrário, o teste falhará com exceção de data na API Java.
...
$body = @{
    ...
    flightDepartureDate = "2026-12-30T14:30:00"
    flightDistance = 3974
} | ConvertTo-Json
